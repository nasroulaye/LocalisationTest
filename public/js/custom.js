...
$(".pt-plus").livequery("click", function(){
	...
	var quantity = parseInt(par.find("[name=quantity]").val());
	var totalId = $(".pt-totalprice");
	var total = parseFloat(totalId.text().replace("F",""));
	var price = parseFloat(par.find(".checked").text().replace("F",""));
	var alltotalprice = parseFloat($(".pt-alltotalprice").text().replace('F',""));
	var extraPrice = 0;
	if(par.find(".pt-removeexratoitem").length){
		par.find(".pt-removeexratoitem").each(function(){
			extraPrice += parseFloat($(this).data("price"));
	...
	if(quantity > 0){
		...
		quantity = quantity+1;
		par.find("[name=quantity]").val(quantity);

		total = (total+price-extraPriceBe+quantity*extraPrice).toFixed(2);

		totalId.text("F"+total);
		$(".pt-alltotalprice").text("F"+(alltotalprice+price-extraPriceBe+quantity*extraPrice).toFixed(2));
